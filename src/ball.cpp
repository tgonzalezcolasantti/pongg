...
void dampen_speed(ball_t* ball, double dt){
    if (SDL_abs(ball->vx) > BALL_SPEED_DEFAULT) ball->vx *= pow(BALL_SPEED_DAMPEN_FACTOR, dt);
    if (SDL_abs(ball->vy) > BALL_SPEED_DEFAULT) ball->vy *= pow(BALL_SPEED_DAMPEN_FACTOR, dt);
